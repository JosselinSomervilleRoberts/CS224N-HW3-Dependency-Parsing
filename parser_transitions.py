...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for i, transition in enumerate(transitions):
            ...
            if len(batch[i].buffer) == 0 and len(batch[i].stack) == 1: # finished parsing
                sentence_idx = batch_indexes[i]
                dependencies[sentence_idx] = batch[i].dependencies
                unfinished_parses.remove(batch[i])
                indexes.remove(sentence_idx)
